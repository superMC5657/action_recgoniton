...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, *target) in enumerate(train_loader):
        ...
        person_target = target[0].to(args.device)
        sex_target = target[1].to(args.device)
        name_target = target[2].to(args.device)
        action_target = target[3].to(args.device)

        # compute output
        person_predict, sex_predict, name_predict, action_predict = model(images)
